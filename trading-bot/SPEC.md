...
| Group | Keys |
|---|---|
| 11.1 Risk and position sizing | 29 (+4 GAP-2-derived: `max_correlated_concurrent_enabled`, `max_correlated_concurrent`, `correlation_threshold`, `correlation_lookback_bars`, `min_correlation_overlap_bars` — all [OUR CHOICE], GAPS.md GAP 2) |
...
`max_entry_distance_pct`) **+ 5 GAP-2-derived** keys (GAPS.md GAP 2, the correlated-exposure
cap) = **252**, which is what `tbot.config.KEY_SPECS` holds.
