...
cfg = Config.load()                     # all 255 SPEC.md §11 defaults
...
`Config` is a **frozen, slotted dataclass with all 255 keys as flat attributes** — `cfg.swing_k`,
...
Every one of the 255 keys is owned by exactly one module below — **the owner is the only module
...
| `max_correlated_concurrent_enabled` | `False` | [OUR CHOICE] - GAPS.md GAP 2; off until a sweep says otherwise |
...
| Key | Default | Source |
|---|---|---|
| `backtest_enforces_concurrency` | `false` | CF-04; S4-R30, S4-R31, S4-C8 — build the PortfolioState his concurrency caps need, so they can fire in the harness |
...
| `min_stop_pct_enforced_at_fill` | `False` | [OUR CHOICE] — a switch; the threshold `min_stop_pct` is CF-06, S7-C8, his |
