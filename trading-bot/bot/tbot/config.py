...
KEY_SPECS: tuple[KeySpec, ...] = (
    KeySpec(
        ...
        group='11.1',
        note='F9 (part B): the stop-side "Amount" on his position tool reads exactly 750 in all three frames where it is visible — BTCUSDT.P 1H (TBOT1 4:11), HYPEUSDT.P 4H (TBOT1 22:59), OMUSDT.P 1H (TBOT1 1:09:19) — three pairs, two timeframes, two exchanges. A CONSTANT CASH RISK per trade, which is exactly the risk-first solve of SPEC.md 8.8 (CF-01/CF-02): quantity comes backwards out of the loss at stop. Corroboration of the MODEL, not of this percentage. Arithmetic this implies, recorded as an INFERENCE and not a fact: a constant 750 at a 4-5% max loss implies a portfolio of roughly 15,000-18,750. Consistent, but his account size is never shown in any frame (F3), so the implication runs the wrong way — no account-size key is derived from it and none is added.',
    ),
    KeySpec(
        ...
        group='11.1',
        note='F9 (part B): the stop-side "Amount" on his position tool reads exactly 750 in all three frames where it is visible — BTCUSDT.P 1H (TBOT1 4:11), HYPEUSDT.P 4H (TBOT1 22:59), OMUSDT.P 1H (TBOT1 1:09:19) — three pairs, two timeframes, two exchanges. A CONSTANT CASH RISK per trade, which is exactly the risk-first solve of SPEC.md 8.8 (CF-01/CF-02): quantity comes backwards out of the loss at stop. Corroboration of the MODEL, not of this percentage. The 4-5% band is the pair this inference uses: a constant 750 across it implies a portfolio of roughly 15,000-18,750. INFERENCE only — his account size is never shown (F3), and no account-size key is added.',
    ),
    KeySpec(
...
def write_default_yaml(path: str | Path) -> Path:
    """Write the full 245-key default set to ``path``, one commented line per key.
    ...
    lines: list[str] = [
        "# tbot default configuration — 245 keys: SPEC.md §11 (234) + the 8 evidence-derived",
